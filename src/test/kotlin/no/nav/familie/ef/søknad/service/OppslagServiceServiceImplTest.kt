...
internal class OppslagServiceServiceImplTest {
    ...
    val pdlClientCredentialClient: PdlClientCredentialClient = mockk()
    ...
    private val oppslagServiceService = OppslagServiceServiceImpl(
            pdlClient,
            pdlClientCredentialClient, regelverkConfig, søkerinfoMapper
    )
    ...
    fun setUp() {
        ...
        every { pdlClientCredentialClient.hentAndreForeldre(any()) } returns (mapOf())
    }
    ...
    fun `SøkerInfo med endring i familierelasjoner skal ikke ha lik hash`() {
        ...
        every { pdlClientCredentialClient.hentBarn(any()) } returns (mapOf(pdlBarn.first to copy))
        every { pdlClientCredentialClient.hentAndreForeldre(any()) } returns mapOf(
    ...
    fun `Skal filtrere bort døde barn`() {
        ...
        val oppslagServiceServiceMedMapper = OppslagServiceServiceImpl(
                pdlClient,
                pdlClientCredentialClient, regelverkConfig, mapper
        )

        every { pdlClientCredentialClient.hentBarn(any()) } returns mapOf(pdlBarn(dødsfall = Dødsfall(LocalDate.MIN)))
        mockHentPersonPdlClient()
    ...
    fun `Skal ikke filtrere bort levende barn`() {
        ...
        every { pdlClientCredentialClient.hentBarn(any()) } returns mapOf(pdlBarn())
        mockHentPersonPdlClient()
        val søkerinfo = oppslagServiceServiceMedMapper.hentSøkerinfo()
    ...
    fun `Skal filtrere bort døde barn og barn i for høy alder`() {
        ...
        every { pdlClientCredentialClient.hentBarn(any()) } returns mapOf(dødtBarn, levendeBarn, barnForHøyAlder)
    ...
    private fun mockPdlHentAnnenForelder(adressebeskyttelseGradering: AdressebeskyttelseGradering) {
        ...
        every { pdlClientCredentialClient.hentAndreForeldre(any()) } returns (mapOf("enIdent" to annenForelder))
