...
@Component
internal class FamilieDokumentSbsClient(private val config: FamilieDokumentConfig,
                                        @Qualifier("restKlientMedApiKey") restTemplate: RestOperations)
    : AbstractPingableRestClient(restTemplate, "familie.dokumentsbs") {

    override val pingUri: URI = config.pingSbsUri

    internal fun vedleggUri(vedleggsId: String) =
            UriComponentsBuilder.fromUri(config.hentVedleggSbsUri).path(vedleggsId).build().toUri()
